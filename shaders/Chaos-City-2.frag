...
{
    //displacement
    vec2 dis = 6.*vec2((.1+.0*iScale)*valuenoise(x-2.-1.e-1*iTime),(.1+.0*iScale)*valuenoise(x.xy-5.-1.e-1*iTime));
    
    //city
    vec3 v = vor(7.*x.xy-iNBeats-dis);
//     float h = .3*clamp(iScale, 0., 1.)*valuenoise(v.yz+1.);
    float h = .1+.3*rand(v.yz+7.+iNBeats)+.3*valuenoise(v.yz+7.+iNBeats)*clamp(3.*iScale, 0., 1.)+.1*valuenoise(v.yz-.4*iTime);
...
#define post(color, uv) \
	{\
    	col = mix(clamp(col,c.yyy,c.xxx), c.xxx, smoothstep(1.5/iResolution.y, -1.5/iResolution.y, stroke(logo(uv-vec2(-.45,.45),.02),.007)));\
